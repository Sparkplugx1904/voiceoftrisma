/* =========================================================
   ...
   1. Cron tiap 5 menit: poll icecast Klikhost lalu CATAT sample
      { t, currentlisteners, streamstatus } ke KV VOT_STREAM_STATS.
   2. Rolling window 6 jam (format padat [t, listeners, status]).
   3. GET /stats = proxy CORS live ke icecast (perilaku lama),
      plus catat sample di background (waitUntil).
   4. GET /stats?history=1 = seluruh time series + snapshot terakhir.

   Key KV: "samples" (time series), "last" (snapshot mentah).
   ========================================================= */
...
/* Catat satu sample ke KV (dedupe per bucket 5 menit, trim 6 jam).
   ...
   bucket yang sama; bucket BARU tetap ditulis. (b) snapshot mentah "last"
   hanya ditulis jika isinya benar-benar berubah. */
...
/* GET /stats?history=1 — time series + snapshot terakhir (dipakai dashboard). */
...
/* GET /stats — proxy live (perilaku lama untuk situs utama) + catat sample di
   background. Kalau query ?history=1 (dipakai dashboard), kembalikan time series. */
